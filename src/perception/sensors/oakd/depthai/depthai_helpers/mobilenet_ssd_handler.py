    ...
    for e in entries_prev:
        ...
        if e[0]['label'] > len(labels):
            ...
            if iteration == 0: # For now we run second-stage only on first detection
                if 'blob_file2' in config['ai'] and 'landmarks-regression-retail-0009' in config['ai']['blob_file2']:
                    # Decode
                    landmarks = []
                ...
                if 'blob_file2' in config['ai'] and 'emotions-recognition-retail-0003' in config['ai']['blob_file2']:
                    # Decode
                    emotion_data = []
                    for i in range(len(e[1])):
